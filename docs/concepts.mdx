...
- **STT (Speech-to-Text)** — transcribes caller audio in real time. Providers: `DeepgramSTT`, `WhisperSTT`, `CartesiaSTT`, `SonioxSTT`, `SpeechmaticsSTT` (Python-only), `AssemblyAISTT` (LeMUR BANT scoring), `XaiSTT`, `FishAudioSTT`. See [STT](/python-sdk/stt).
...
- **TTS (Text-to-Speech)** — synthesizes the reply audio. Providers: `ElevenLabsTTS`, `OpenAITTS`, `CartesiaTTS`, `RimeTTS`, `LMNTTTS`, `XaiTTS`, `FishAudioTTS` (WebSocket MessagePack), `SarvamBulbulTTS`. See [TTS](/python-sdk/tts).
- **Avatar & WebRTC Infrastructure** — Real-time interactive avatar video generation (`SimliTTS` / Local GPU pool) and WebRTC media transport (`LiveKit`).
- **Visio Meeting Engine** — Omnichannel bot dispatch for Zoom, Google Meet, MS Teams, and Webex (`Recall.ai` integration).
